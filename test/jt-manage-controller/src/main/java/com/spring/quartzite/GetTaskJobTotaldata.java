...
public class GetTaskJobTotaldata {
	@Autowired
	private   StatisticsService  settinService;
	private static final Logger log = Logger.getLogger(GetTaskJobTotaldata.class);
	//定时器
	public void work() {
		log.debug("开始_执行总统计");
	...
	public void totalDate() {
		...
		ttd.setDateInt(Integer.valueOf(sdf.format(new Date())));		
